...
impl PersistenceManager {
    ...
    pub async fn new(
        ...
        let aof_writer = if config.aof_enabled {
            let writer = AofWriter::new(&aof_file, config.aof_fsync)
                .await
        ...
        let manager = Self { inner, aof_channel };
    ...
    pub async fn save(&self) -> crate::Result<()> {
        ...
        match rdb::bgsave(&self.inner.store, &self.inner.rdb_file).await {
            Ok(()) => {
                ...
                let file_size = tokio::fs::metadata(&self.inner.rdb_file)
                    .await
                    .map(|m| m.len())
                    .ok();
                ...
                *self.inner.last_save.write().await = Instant::now();
                ...
                self.inner.metrics.record_rdb_save();
                info!(elapsed_ms = elapsed.as_millis(), file_size, "RDB save completed");
                Ok(())
            }
            Err(e) => {
                let mut stats = self.inner.stats.write().await;
    ...
    ///
    /// Mirrors Redis semantics: if AOF is enabled and a file exists, AOF is the
    /// authoritative source and RDB is ignored. Otherwise fall back to RDB.
    pub async fn load_on_startup(&self) -> crate::Result<()> {
        if self.inner.config.aof_enabled && self.inner.aof_file.exists() {
            ...
            let count = super::aof::AofReplayer::replay_with_metrics(
                &self.inner.store,
                &self.inner.aof_file,
                &self.inner.metrics,
            )
        ...
        if self.inner.config.rdb_enabled && self.inner.rdb_file.exists() {
            ...
            rdb::load(&self.inner.store, &self.inner.rdb_file)
                .await
                .map_err(|e| crate::RedisError::Storage(e.to_string()))?;
    ...
    pub async fn rewrite_aof(&self) -> crate::Result<u64> {
        ...
        let count = super::aof::rewrite_aof(&self.inner.store, &self.inner.aof_file, &temp_path)
            .await
            .map_err(|e| crate::RedisError::Storage(e.to_string()))?;

        // Update file size in stats
        let file_size = tokio::fs::metadata(&self.inner.aof_file)
            .await
            .map(|m| m.len())
            .ok();
