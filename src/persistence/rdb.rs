    ...
    pub async fn save_to_file(store: &Store, path: &Path) -> Result<()> {
        ...
        for (key, value) in &data {
            ...
            if let Some(expire_at) = expirations.get(key) {
                ...
                Self::write_u64_ms(&mut file, ttl_ms).await?;
            }
        ...
        loop {
            ...
            match opcode {
                ...
                OP_EXPIRE_TIME => {
                    ...
                    let expire_s = u32::from_le_bytes([buf[pos], buf[pos+1], buf[pos+2], buf[pos+3]]) as u64;
                    pos += 4;
        ...
        match enc_type {
            ...
            2 => {
                ...
                let val = u64::from_be_bytes([
                    ...
                    buf[pos+5], buf[pos+6], buf[pos+7], buf[pos+8]
                ]);
                Ok((val, pos + 9))
            }
    ...
    fn read_value(buf: &[u8], pos: usize, value_type: u8) -> Result<(StoredValue, usize)> {
        match value_type {
            ...
            TYPE_ZSET => {
                ...
                let mut tree: BTreeMap<ordered_float::OrderedFloat<f64>, HashSet<Bytes>> = BTreeMap::new();
...
mod tests {
    ...
    async fn test_rdb_save_load() {
        ...
        store.h_set("hash1".to_string(), "field1".to_string(), Bytes::from("h_value1"));
        ...
        // Save
        RdbSerializer::save_to_file(&store, &temp_path).await.unwrap();
        
        // Load into new store
        let new_store = Store::new();
        RdbDeserializer::load_from_file(&new_store, &temp_path).await.unwrap();
        
        // Verify
        assert_eq!(new_store.get("key1"), Some(Bytes::from("value1")));
        assert_eq!(new_store.h_get("hash1","field1"), Some(Bytes::from("h_value1")));
        assert_eq!(new_store.z_score("zset1", &Bytes::from("member1")), Some(1.0));
